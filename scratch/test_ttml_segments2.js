// scratch/test_ttml_segments2.js
async function main() {
  ...
  ws.onopen = async () => {
    ...
    const testRes = await send('Runtime.evaluate', {
      expression: `(async () => {
        ...
        const p = container?.ui?.getControls()?.getPlayer() || window._player;
        const uri = p?.getAssetUri ? p.getAssetUri() : null;
        if (!uri) return 'No uri from player';

        const base = uri.substring(0, uri.lastIndexOf('/') + 1) + 'dash/';
        
        // Let's test 10 segment numbers around current time (current time ~800s => 800 / 5.76 = ~138)
        const currentSegNum = Math.floor(800 / 5.76);
        const results = [];

        // Check segments from 135 to 145
    ...
    console.log('TTML segments test 2:', JSON.stringify(testRes, null, 2));
    ws.close();
  };
}
